...
export default function QuizScreen() {
  ...
  const handleAnswer = (option: string) => {
    ...
    if (step < QUIZ_QUESTIONS.length - 1) {
      ...
      addHistory({ type: 'Quiz', icon: '🎯', main: `Quiz — Perfil ${result.name}`, meta: 'Resultado identificado' });
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.progressText}>Pergunta {step + 1} de {QUIZ_QUESTIONS.length}</Text>
...
const styles = StyleSheet.create({
  ...
  content: { padding: 20 },
  progressBar: { height: 4, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 2, marginBottom: 12, overflow: 'hidden' },
  progressFill: { height: '100%', backgroundColor: '#00a3e0', borderRadius: 2 },
  progressText: { fontSize: 11, color: 'rgba(255,255,255,0.4)', marginBottom: 20 },
  question: { fontSize: 22, fontWeight: 'bold', color: 'white', marginBottom: 24, lineHeight: 28 },
  option: {
  ...
  optionText: { fontSize: 14, color: 'rgba(255,255,255,0.75)', fontWeight: '500' },
});