      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.content}>
          ...
          {OBJECTIONS.map(o => (
            <TouchableOpacity
              key={o.id}
              style={[styles.objChip, selected === o.id && styles.objChipActive]}
              onPress={() => setSelected(o.id)}
            >
              <Text style={[styles.objChipText, selected === o.id && styles.objChipTextActive]}>{o.label}</Text>
            </TouchableOpacity>
          ))}

          {obj && (
            <>
              <LinearGradient colors={['#001a4d', '#003078']} style={styles.objCard}>
              ...
              <View style={styles.card}>
                ...
                <View style={styles.supportList}>
                  {obj.support.map(s => (
                    <View key={s} style={styles.supportTag}><Text style={styles.supportText}>{s}</Text></View>
                  ))}
                </View>
              </View>

              <View style={[styles.card, styles.careCard]}>
                <Text style={[styles.cardTitle, styles.careTitle]}>⚠️ Ponto de Cuidado</Text>
          ...
          {!selected && (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySub}>Veja como responder cada objeção do cliente</Text>
            </View>
          )}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  objChipActive: { backgroundColor: 'rgba(0,48,120,0.5)', borderColor: '#00a3e0' },
  objChipText: { fontSize: 13, color: 'rgba(255,255,255,0.7)', fontWeight: '500' },
  objChipTextActive: { color: 'white' },
  objCard: { borderRadius: 16, padding: 20, marginVertical: 16, borderWidth: 1, borderColor: 'rgba(0,163,224,0.3)' },
  objLabel: { fontSize: 10, color: '#ff3d00', textTransform: 'uppercase', letterSpacing: 2, marginBottom: 8 },
  ...
  supportTag: { backgroundColor: 'rgba(0,163,224,0.15)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6 },
  supportText: { fontSize: 11, fontWeight: 'bold', color: '#00a3e0', textTransform: 'uppercase' },
  careCard: { borderColor: 'rgba(255,214,0,0.2)', backgroundColor: 'rgba(255,214,0,0.04)' },
